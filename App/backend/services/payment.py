...
# Load .env file as fallback (do NOT override platform-injected env vars)
...
async def initialize_stripe():
    ...
    if not stripe_key:
        ...
        raise CheckoutError(
            "Stripe is not configured. Please publish the site to activate payments, or check that STRIPE_SECRET_KEY is set.",
            error_type="authentication",
            is_retryable=False,
            fixable=True,
            fix_suggestion="Set STRIPE_SECRET_KEY environment variable or publish the site to inject platform credentials",
        )
...
class PaymentService:
    ...
    async def _auto_reload_stripe_config():
        ...
        Re-reads STRIPE_SECRET_KEY from environment on each call to pick up
        keys injected after process start (e.g. platform deployment).
        """
        # Always re-read from env in case it was injected after startup
        fresh_key = os.environ.get("STRIPE_SECRET_KEY", "")
        if fresh_key and stripe.api_key == fresh_key:
            return  # Already configured with current key

        if fresh_key:
            # Key available (possibly newly injected) — set it
            stripe.api_key = fresh_key
            logger.info("Stripe API key loaded from environment")
            return

        # No key in env — try settings (which reads .env file)
