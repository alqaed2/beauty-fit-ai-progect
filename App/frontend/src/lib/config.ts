...
// Use empty string so requests become same-origin relative paths
// (e.g. "/api/v1/..."). In dev the Vite server proxies `/api` to the
// backend on :8000; in production the frontend and backend share origin.
// Hard-coding http://127.0.0.1:8000 here breaks any preview served from
// a non-localhost host, surfacing as "Network Error" in the browser.
const defaultConfig = {
  API_BASE_URL: '',
};
...
export function getConfig() {
  ...
  if (runtimeConfig) {
    // Detect unresolved platform placeholders like $$backend_domain$$
    if (runtimeConfig.API_BASE_URL && runtimeConfig.API_BASE_URL.includes('$$')) {
      console.log('Runtime config contains unresolved placeholder, falling back to relative URLs');
      return defaultConfig;
    }
  ...
  if (import.meta.env.VITE_API_BASE_URL) {
    // Check for unresolved platform placeholders like $$backend_domain$$
    if (import.meta.env.VITE_API_BASE_URL.includes('$$')) {
      console.log('Vite env contains unresolved placeholder, falling back to relative URLs');
      return defaultConfig;
    }
    const viteConfig = {
